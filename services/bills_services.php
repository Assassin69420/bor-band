...
function get_bill(string $bill_id, mysqli $db)
{
		$sql = "SELECT bill_id, user_id, amount, due_date, paid_date, related_plan, cgst_percentage, sgst_percentage, coalesce(plan_name, service_name) as ps_name, U.name as username, U.id as account_id
						FROM bills B
						INNER JOIN useraccount U ON U.id = B.user_id
						LEFT JOIN internet_plans P ON P.id = B.related_plan
						LEFT JOIN services S ON S.id = B.related_service
